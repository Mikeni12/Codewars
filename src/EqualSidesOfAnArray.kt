...
of the array, the sum of left side of the index ({1,2,3}) and the sum of the right side of the index ({3,2,1})
both equal 6.

Let's look at another one.
You are given the array {1,100,50,-51,1,1}: Your function will return the index 1, because at the 1st position of
the array, the sum of left side of the index ({1}) and the sum of the right side of the index ({50,-51,1,1})
both equal 1.
...
object EqualSidesOfAnArray {
    fun findEvenIndex(arr: IntArray): Int {
        ...
        for (i in arr.indices) {
            val sumLeft = arr.slice(0..i).reduce { acc, i -> acc + i }
            val sumRight = arr.slice(arr.size - 1 downTo i).reduce { acc, i -> acc + i }
            if (sumLeft == sumRight && index < i) index = i
        }
        return index
    }
