/*
In the drawing below we have a part of the Pascal's triangle, lines are numbered from zero (top). The left diagonal in
pale blue with only numbers equal to 1 is diagonal zero, then in dark green (1, 2, 3, 4, 5, 6, 7) is diagonal 1, then in
...
object Diagonal {
    fun diagonal(n: Int, p: Int): BigInteger = when {
        n == 0 && p == 0 -> BigInteger.ONE
        else -> {
            var ans = BigInteger.ONE
            for (i in n.inc() - p.inc() + 1..n.inc()) ans *= i.toBigInteger()
            val factorial = (1..p.inc()).reduce(Int::times).toBigInteger()
            ans / factorial
        }
    }
}
...
class DiagonalTest {
    @Test
    fun basicTests() {
        assertEquals(BigInteger.valueOf(8), Diagonal.diagonal(7, 0))
