...
fun rangeExtraction(arr: IntArray): String = arr
    .sorted()
    .fold(mutableListOf<MutableList<Int>>()) { acc, num ->
        if (acc.isEmpty() || acc.last().last() != num - 1) acc.add(mutableListOf(num))
        else acc.last().add(num)
        acc
    }
    .joinToString(",") { if (it.size < 3) it.joinToString(",") else "${it.first()}-${it.last()}" }
