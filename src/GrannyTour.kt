...
object Tour {
    ...
    fun tour(arrFriends: Array<String>, ftwns: Array<Array<String>>, h: Map<String, Double>): Int {
        val map = ftwns.filter { arrFriends.contains(it.first()) }.map { it.component2() to h[it.component2()]!! }
        var nDistance = map.first().second + map.last().second
        for (friend in map.size - 1 downTo 1) {
            nDistance += Math.sqrt(
                Math.pow(map.elementAt(friend).second, 2.0) - Math.pow(map.elementAt(friend - 1).second, 2.0)
            )
        }
        return nDistance.toInt()
    }
}
