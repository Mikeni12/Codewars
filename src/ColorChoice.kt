...
fun checkchoose(m: Long, n: Int): Long =
    (1L until n)
        .filter { x -> BigInteger.valueOf(m) == BigInteger.valueOf(n.toLong())
            .reduce(BigInteger.valueOf(n - x)).divide(BigInteger.valueOf(x.factorial()))}
        .elementAtOrElse(0) { -1L }
fun Long.factorial(): Long = if (this >= 1) this * (this - 1).factorial() else 1
fun BigInteger.reduce(x: BigInteger): BigInteger = if (this > x) this.times((this - BigInteger.ONE).reduce(x)) else BigInteger.ONE
...
class CheckchooseTest {
    ...
    fun BasicTests() {
        ...
        assertEquals(-1, checkchoose(36, 7))
        assertEquals(2, checkchoose(528, 33))
        val a = 47129212243960
        assertEquals(20, checkchoose(a, 50))
        assertEquals(-1, checkchoose(47129212243961, 50))
    }
